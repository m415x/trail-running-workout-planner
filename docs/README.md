...
`docs/handoffs/` is temporary operational state, not an archive. The latest story-closure handoff is [`handoffs/kan-411.md`](handoffs/kan-411.md), covering date-derived session microcycles, multi-group validation, ES/EN reconciliation and the pending integration into `dev`. [`handoffs/kan-410.md`](handoffs/kan-410.md) remains the prior integrated baseline until KAN-411 integration is verified. Earlier KAN-407/KAN-375/KAN-409 handoffs remain only as recent navigation context until consolidated/removed. When no story is active, the most recent completed handoff is a bootstrap aid rather than execution authority: new work must still verify current `dev`, read only relevant durable domain docs, and then consult current Jira.
...
Post-KAN-410 integrated baseline: the workout-type catalog is canonical, Sessions create/list/calendar/detail/edit and SessionForm use the modular ES/EN boundary, server-side Session errors cross the UI boundary as stable codes, and microcycle options expose structural data for localized composition. KAN-410 closed with 1183/1183 tests, lint/typecheck/i18n/build GREEN and an ES/EN walkthrough; its branch was integrated into `dev` at `5dfb36d035ce38e724a18b78c7b77371f1cc0bd8`. KAN-411 date-derived microcycle resolution has passed story-branch gates and manual walkthrough; see its handoff for the integration status. Jira-owned deferred work includes KAN-412 legacy intensity-percentage reconciliation and KAN-413 `durationMin` semantics. Treat those keys as navigation pointers only until their current Jira state and complete issue content are read.
