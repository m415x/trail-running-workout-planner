...
`docs/handoffs/` is temporary operational state, not an archive. The current story-closure handoff is [`handoffs/kan-413.md`](handoffs/kan-413.md), covering the explicit planned group-duration contract, variable-terrain presentation, plan-real separation and the KAN-454 seed-ownership regression found during walkthrough. [`handoffs/kan-412.md`](handoffs/kan-412.md) records the immediately preceding integrated reference-percentage story. Earlier handoffs are navigation context only and may be consolidated/removed. When no story is active, the most recent completed handoff is a bootstrap aid rather than execution authority: new work must still verify current `dev`, read only relevant durable domain docs, and then consult current Jira.
