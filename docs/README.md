...
`docs/handoffs/` is temporary operational state, not an archive. The current story-closure handoff is [`handoffs/kan-412.md`](handoffs/kan-412.md), covering the canonical reference-percentage contract, SQLite/Supabase deployment boundary, ES/EN walkthrough and the pending integration into `dev`. [`handoffs/kan-411.md`](handoffs/kan-411.md) records the immediately preceding integrated story. Earlier handoffs are navigation context only and may be consolidated/removed. When no story is active, the most recent completed handoff is a bootstrap aid rather than execution authority: new work must still verify current `dev`, read only relevant durable domain docs, and then consult current Jira.
...
KAN-411 is the preceding integrated Sessions baseline. KAN-412 implementation on `kan-412-reference-percentage-contract` has passed its local full gate and ES/EN Coach/Athlete walkthrough; see its handoff for exact evidence and integration status. Its canonical contract is `reference_percentage` / `referencePercentage`, with the closed session-prescription options 50/60/70/80/90/100/110/115/120 and Z1-Z5 remaining independent. Supabase production migration 0022 is deliberately deferred to controlled deployment and must not be described as remotely applied. Jira-owned deferred work includes KAN-413 `durationMin` semantics. Treat issue keys as navigation pointers and read current Jira before execution.
