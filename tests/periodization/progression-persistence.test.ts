...
describe('persistencia de la progresión de carga', () => {
  ...
  it('conserva fechas, tipo y notas editados al guardar un volumen manual', () => {
    const planning = createPlanning()
    persistProgression({ groupTrainingPlanId: 'plan-1', macrocycleId: 'macro-1', planning, database })
    const week = database.select().from(microcycles).where(eq(microcycles.weekNumber, 2)).get()

    assert.ok(week)
    database.update(microcycles).set({
      startDate: '2026-01-13', endDate: '2026-01-19', type: 'shock',
      notes: 'Ajuste deliberado del profesor', targetVolumeKm: 34.5, targetVolumeSource: 'manual',
    ...
    const regenerated = createPlanning([{
      ...
      targetElevationGain: week.targetElevationGain, targetElevationSource: week.targetElevationSource,
    }])
    persistProgression({ groupTrainingPlanId: 'plan-1', macrocycleId: 'macro-1', planning: regenerated, database })

    const savedWeek = database.select().from(microcycles).where(eq(microcycles.id, week.id)).get()
  ...
  it('persiste el D+ manual en regeneraciones sucesivas sin duplicar semanas', () => {
    const planning = createPlanning()
    persistProgression({ groupTrainingPlanId: 'plan-1', macrocycleId: 'macro-1', planning, database })
    const week = database.select().from(microcycles).where(eq(microcycles.weekNumber, 4)).get()

    assert.ok(week)
    database.update(microcycles).set({ targetElevationGain: 1_250, targetElevationSource: 'manual' })
      .where(eq(microcycles.id, week.id)).run()

    const regenerated = createPlanning([{
      id: week.id, weekNumber: week.weekNumber, targetVolumeKm: week.targetVolumeKm,
      targetVolumeSource: week.targetVolumeSource, targetElevationGain: 1_250, targetElevationSource: 'manual',
    }])
  ...
  it('convierte las últimas semanas existentes en taper y carrera y guarda el conteo', () => {
    ...
    persistProgression({ groupTrainingPlanId: 'plan-1', macrocycleId: 'macro-1', planning: initialPlanning, database })
    const existingMicrocycles = database.select().from(microcycles).all().map((week) => ({
      id: week.id, weekNumber: week.weekNumber, targetVolumeKm: week.targetVolumeKm,
      targetVolumeSource: week.targetVolumeSource, targetElevationGain: week.targetElevationGain,
      targetElevationSource: week.targetElevationSource,
    }))
    const racePlanning = buildLoadProgressionPreview({
      title: 'Macrociclo S2', startDate: '2026-01-05', endDate: '2026-03-01',
      loadStrategy: suggestLoadStrategy('S2', 'race'),
      targetRace: { name: 'Carrera objetivo', distanceKm: 21, elevationGain: 900 }, existingMicrocycles,
    }).planning

    persistProgression({ groupTrainingPlanId: 'plan-1', macrocycleId: 'macro-1', planning: racePlanning, database })

    const savedMacrocycle = database.select().from(macrocycles).where(eq(macrocycles.id, 'macro-1')).get()
    const competitiveMesocycle = database.select().from(mesocycles).where(eq(mesocycles.period, 'competitive')).get()
    const competitiveWeeks = database.select().from(microcycles)
      .where(eq(microcycles.mesocycleId, competitiveMesocycle!.id)).all()
  ...
  it('rechaza una propuesta inválida antes de realizar escrituras', () => {
    ...
    assert.throws(() => persistProgression({ groupTrainingPlanId: 'plan-1', macrocycleId: 'macro-1', planning, database }), /semana 1 está duplicada/)
  ...
  it('desactiva semanas de entrenamiento obsoletas sin tocar bloques protegidos', () => {
    ...
    const shortenedPlanning = { ...planning, mesocycles: planning.mesocycles.slice(0, 1) }
    const result = persistProgression({ groupTrainingPlanId: 'plan-1', macrocycleId: 'macro-1', planning: shortenedPlanning, database })

    const staleWeeks = database.select().from(microcycles).all().filter((week) => week.weekNumber >= 5 && week.weekNumber <= 8)
    const protectedWeek = database.select().from(microcycles).where(eq(microcycles.id, 'race-week')).get()
    const staleMesocycle = database.select().from(mesocycles).where(eq(mesocycles.number, 2)).get()
  ...
  it('rechaza una propuesta que invade un bloque competitivo sin escribir cambios', () => {
    ...
    assert.throws(() => persistProgression({ groupTrainingPlanId: 'plan-1', macrocycleId: 'macro-1', planning, database }), /reservado para un bloque competitivo o de transición/)
...
function seedPlan(testDatabase: TestDatabase) {
  testDatabase.insert(groupTrainingPlans).values({ id: 'plan-1', groupId: 'group-1', title: 'Plan S2' }).run()
  testDatabase.insert(macrocycles).values({
    id: 'macro-1', groupTrainingPlanId: 'plan-1', title: 'Macrociclo S2',
    startDate: '2026-01-05', endDate: '2026-03-01',
  }).run()
}
...
function createPlanning(existingMicrocycles: Parameters<typeof buildLoadProgressionPreview>[0]['existingMicrocycles'] = []) {
  return buildLoadProgressionPreview({
    title: 'Macrociclo S2', startDate: '2026-01-05', endDate: '2026-03-01',
    loadStrategy: suggestLoadStrategy('S2', 'base'), existingMicrocycles,
  }).planning
}

function insertProtectedBlock(testDatabase: TestDatabase, number = 3) {
  testDatabase.insert(mesocycles).values({
    id: 'race-block', macrocycleId: 'macro-1', title: 'Competencia', number,
    period: 'competitive', objective: 'Carrera objetivo',
  }).run()
  testDatabase.insert(microcycles).values({
    id: 'race-week', mesocycleId: 'race-block', weekNumber: 12, type: 'race',
    startDate: '2026-03-23', endDate: '2026-03-29', targetVolumeKm: 42,
    targetVolumeSource: 'manual', notes: 'No modificar',
  }).run()
}
