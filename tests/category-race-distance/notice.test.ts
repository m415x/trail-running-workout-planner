...
import es from '@/messages/es/competitions/race-distance-policy.json'
import en from '@/messages/en/competitions/race-distance-policy.json'
...
describe('race distance notices', () => {
  it('shows cause, range, tolerance, and continuation in Spanish', () => {
  ...
  it('renders the notice in English without missing messages', () => {
  ...
  it('hides notices for compatible, unrestricted, and missing races', () => {
    for (const [category, distance] of [
      ['S', 12],
      ['S', 16.5],
      ['H', 21.0975],
      ['M', 42.195],
  ...
  it('distinguishes undefined policy, compatibility, and validation errors', () => {
  ...
  it('recomputes presentation when distance changes without stale notices', () => {
