...
function volumeSignal(
  ...
  return {
    ...
    current: pattern === 'insufficient_data' ? null : {
      ...
      magnitude: {
        planned: 40,
        realized: pattern === 'within_plan' ? 40 : 48,
...
function adherence(
  ...
  return {
    ...
    points: state === 'available' ? [
      { window: { startDate: '2026-08-01', endDate: '2026-08-07' }, adherencePercent: 80, coveragePercent: 100 },
      { window: { startDate: '2026-08-08', endDate: '2026-08-14' }, adherencePercent: 68, coveragePercent: 100 },
    ] : [],
    rule: {
      version: 'adherence-v1',
      highFrequencyMinimum: 85,
      mediumFrequencyMinimum: 70,
      highDimensionMinimum: 90,
      mediumDimensionMinimum: 75,
...
describe('integrated athlete training response review', () => {
  it('produces priority from compatible independent evidence and preserves adherence as context', () => {
    const result = buildAthleteTrainingResponseReview({
      ...
      internalLoad: internalLoad('recent_load_above_baseline'),
      adherence: adherence(),
    })

    assert.equal(result.attention, 'priority')
    assert.equal(result.temporalCompatibility, 'compatible')
    assert.deepEqual(result.contributors.map(({ domain }) => domain), [
      'systematic_volume',
      'internal_load',
      'adherence',
    ])
  ...
  it('preserves known review evidence when another domain is insufficient', () => {
    const result = buildAthleteTrainingResponseReview({
      systematicVolume: volumeAssessment('systematic_excess'),
      internalLoad: internalLoad('insufficient_data'),
      adherence: adherence('insufficient_data'),
    })
  ...
  it('does not let declining adherence elevate otherwise normal evidence', () => {
    const result = buildAthleteTrainingResponseReview({
      ...
      internalLoad: internalLoad('stable_or_lower'),
      adherence: adherence('available', 'declining'),
    })
