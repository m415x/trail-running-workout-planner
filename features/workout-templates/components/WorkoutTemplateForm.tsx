...
import { createWorkoutTemplate, type WorkoutTemplateFormState } from '@/app/actions/workout-template-actions'
import type { WorkoutTemplateCategory, WorkoutType } from '@/types'
...
export function WorkoutTemplateForm({ locale, locations }: WorkoutTemplateFormProps) {
  ...
  const [state, formAction, pending] = useActionState(createWorkoutTemplate, initialState)
  const [intensityMethod, setIntensityMethod] = useState('')
  const templatesPath = locale === 'es' ? '/dashboard/templates' : `/${locale}/dashboard/templates`
  const savedValue = (name: string) => state.values?.[name]
    ...
    <form key={state.submissionKey ?? 0} action={formAction} className='space-y-6'>
      ...
      <fieldset className='space-y-4 rounded-lg border p-4'>
        ...
        <div className='grid gap-4 sm:grid-cols-2'>
          ...
          <SelectField label={t('identity.type')} name='type' defaultValue={savedValue('type')} required error={state.fieldErrors?.type}>
            <option value=''>{t('identity.selectType')}</option>
            {workoutTypes.map((type) => <option key={type} value={type}>{workoutTypeLabel(type)}</option>)}
          </SelectField>
        </div>
        <div className='grid gap-4 sm:grid-cols-2'>
          <SelectField label={t('identity.category')} name='category' defaultValue={savedValue('category')} required error={state.fieldErrors?.category}>
      ...
      <fieldset className='space-y-4 rounded-lg border p-4'>
        ...
        <TextAreaField label={t('structure.preliminary')} name='preliminaryExercises' rows={2} defaultValue={savedValue('preliminaryExercises')} placeholder={t('structure.preliminaryPlaceholder')} />
        <TextAreaField label={t('structure.warmup')} name='warmup' rows={2} defaultValue={savedValue('warmup')} placeholder={t('structure.warmupPlaceholder')} />
        <TextAreaField label={t('structure.mainBlock')} name='mainBlock' rows={3} defaultValue={savedValue('mainBlock')} placeholder={t('structure.mainBlockPlaceholder')} />
        <TextAreaField label={t('structure.cooldown')} name='cooldown' rows={2} defaultValue={savedValue('cooldown')} placeholder={t('structure.cooldownPlaceholder')} />
      ...
      <fieldset className='space-y-4 rounded-lg border p-4'>
        ...
        <div className='grid gap-4 sm:grid-cols-2'>
          <SelectField label={t('session.location')} name='locationKey' defaultValue={savedValue('locationKey')}>
        ...
        <TextAreaField label={t('session.notes')} name='notes' rows={3} defaultValue={savedValue('notes')} placeholder={t('session.notesPlaceholder')} />
      </fieldset>
      ...
      <fieldset className='space-y-4 rounded-lg border p-4'>
        ...
        {intensityMethod === 'hr_zone' && (
          <SelectField label={t('prescription.zone')} name='zone' defaultValue={savedValue('zone')} required error={state.fieldErrors?.intensity}>
        ...
        <TextAreaField label={t('prescription.notes')} name='prescriptionNotes' rows={3} defaultValue={savedValue('prescriptionNotes')} placeholder={t('prescription.notesPlaceholder')} />
      </fieldset>

      <div className='flex justify-end gap-2'>
        <Link href={templatesPath} className={buttonVariants({ variant: 'outline' })}>{t('cancel')}</Link>
        <Button type='submit' disabled={pending}>{pending ? t('saving') : t('create')}</Button>
      </div>
    </form>
  )
}
