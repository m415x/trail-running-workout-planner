  ...
  return (
    <div className='mx-auto w-full max-w-5xl space-y-6'>
      ...
      <div className='grid gap-6 md:grid-cols-2'>
        ...
        <Card><CardHeader><CardTitle>{t('personalData')}</CardTitle></CardHeader><CardContent><dl className='grid gap-5 sm:grid-cols-2'><DetailItem label='DNI' value={athlete.dni} /><DetailItem label={t('birthDate')} value={formatDate(athlete.birthday)} /><DetailItem label={t('nickname')} value={athlete.nickName} /><div><dt className='text-sm text-muted-foreground'>{t('group')}</dt><dd className='mt-1'>{groupCode ? <Badge variant='secondary'>{groupCode}</Badge> : <Badge variant='outline'>{t('noGroup')}</Badge>}</dd></div></dl></CardContent></Card>
        ...
        <Card className='md:col-span-2'><CardHeader><div className='flex flex-wrap items-center justify-between gap-3'><CardTitle className='flex items-center gap-2'><Target className='size-5' />{t('goals')}</CardTitle><Link href={newGoalPath} className={buttonVariants({ variant: 'outline', size: 'sm' })}>{t('newGoal')}</Link></div></CardHeader><CardContent>{goals.length === 0 ? <p className='rounded-lg border border-dashed p-4 text-sm text-muted-foreground'>{t('noGoals')}</p> : <div className='space-y-3'>{goals.map((goal) => <div key={goal.id} className='rounded-lg border p-4'><div className='grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-start'><div className='min-w-0'><div className='flex flex-wrap items-center gap-2'><p className='font-medium'>{goal.title}</p><Badge variant={goal.status === 'draft' ? 'outline' : 'secondary'}>{GOAL_STATUS_LABELS[goal.status] ?? goal.status}</Badge></div>{goal.type === 'race' && goal.raceName && goal.raceName !== goal.title && <p className='mt-1 text-sm text-muted-foreground'>{goal.raceName}</p>}{goal.description && <p className='mt-2 text-sm'>{goal.description}</p>}{goal.notes && <p className='mt-2 text-sm text-muted-foreground'>{goal.notes}</p>}</div><dl className='grid grid-cols-2 gap-x-6 gap-y-3 text-sm sm:grid-cols-3 lg:min-w-[24rem] lg:text-right'><div><dt className='text-muted-foreground'>{t('date')}</dt><dd className='mt-0.5 font-medium'>{formatDate(goal.targetDate)}</dd></div>{goal.type === 'race' && <><div><dt className='text-muted-foreground'>{t('distance')}</dt><dd className='mt-0.5 font-medium'>{goal.raceDistanceKm == null ? '—' : `${goal.raceDistanceKm} km`}</dd></div><div><dt className='text-muted-foreground'>D+</dt><dd className='mt-0.5 font-medium'>{goal.raceElevationGain == null ? '—' : `+${goal.raceElevationGain} m`}</dd></div></>}</dl></div></div>)}</div>}</CardContent></Card>
        ...
        <Card className='md:col-span-2'><CardHeader><CardTitle className='flex items-center gap-2'><ShieldAlert className='size-5' />{t('emergencyContact')}</CardTitle></CardHeader><CardContent><dl className='grid gap-5 sm:grid-cols-2'><DetailItem label={t('contact')} value={athlete.emergencyContact} /><DetailItem label={t('phone')} value={athlete.emergencyPhone} /></dl></CardContent></Card>
      </div>
    </div>
  )
}
