...
function AthleteTrack1000mPanel({ locale, events }: { locale: string; events: Array<{ id: string; scheduledAt: string }> }) {
  return <article className='rounded-2xl border bg-card p-5 shadow-sm md:col-span-2'><h2 className='font-heading text-lg font-bold'>Test 1000 m</h2><p className='mt-1 text-sm text-muted-foreground'>Registrá un test oficial programado o un intento autogestionado.</p><AthleteTrack1000mForm locale={locale} events={events} /></article>
}
...
export default async function StatsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <section className='mx-auto w-full max-w-5xl px-4 py-6 sm:px-6 lg:py-8'>
    ...
    <div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
      <AthleteTrack1000mPanel locale={locale} events={testEventsResult.success ? testEventsResult.data : []} />
