...
/** Append-only audit trail for manual corrections to realized evidence. */
export const workoutLogCorrections = pgTable(
  'workout_log_corrections',
  {
    ...
    reason: text('reason'),
    beforeSnapshot: jsonb('before_snapshot').$type<RealizedTrainingCorrectionSnapshot>().notNull(),
    afterSnapshot: jsonb('after_snapshot').$type<RealizedTrainingCorrectionSnapshot>().notNull(),
  },
  (table) => [
